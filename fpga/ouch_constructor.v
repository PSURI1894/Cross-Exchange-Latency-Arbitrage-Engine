...
module ouch_constructor (
    input wire clk,
    input wire rst_n,
    input wire trigger_trade,
    input wire trade_side,
    input wire [31:0] target_stock_id,
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (state) 
                ...
                4: begin tx_data <= {32'h0, target_stock_id}; tx_data_valid <= 1; tx_eof <= 1; order_token_counter <= order_token_counter + 1; state <= 0; end
            endcase
        end
    end
endmodule
